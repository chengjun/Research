    ...
    class APIMethod(object):
        ...
        def build_parameters(self, args, kargs):
            ...
            for idx, arg in enumerate(args):
                try:
                    self.parameters[self.allowed_param[idx]] = str(arg)
                except IndexError:
        ...
        def execute(self):
            ...
            if len(self.parameters):
                if self.method == 'GET' or self.method == 'DELETE':
                    url = '%s?%s' % (url, urllib.parse.urlencode(self.parameters))
                else:
                    self.headers.setdefault("User-Agent","python")
                    if self.post_data is None:
                        self.headers.setdefault("Accept","text/html")                        
                        self.headers.setdefault("Content-Type","application/x-www-form-urlencoded")
                        self.post_data = urllib.parse.urlencode(self.parameters)
            ...
            while retries_performed < self.retry_count + 1:
                ...
                try:
                    ...
                    raise WeibopError('Failed to send request: %s' % str(e) + ".<url=" + str(url) +",self.headers="+ str(self.headers)+'>')
