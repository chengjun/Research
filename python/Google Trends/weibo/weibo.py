...
class Api(object):
  ...
  def GetTrendsCurrent(self, exclude=None):
    ...
    url  = '%s/trends/hourly.json' % self.base_url
    json = self._FetchUrl(url, parameters=parameters)
    data = self._ParseAndCheckTwitter(json)
  ...
  def GetFriendsTimeline(self,
    ...
    if not user and not self._oauth_consumer:
      raise WeiboError("User must be specified if API is not authenticated.")
    ...
    if count is not None:
      try:
        if int(count) > 100:
          raise WeiboError("'count' may not be greater than 100")
      except ValueError:
        raise WeiboError("'count' must be an integer")
      parameters['count'] = count
    if page is not None:
      try:
        parameters['page'] = int(page)
      except ValueError:
        raise WeiboError("'page' must be an integer")
    if since_id:
      parameters['since_id'] = since_id
    if retweets:
  ...
  def GetUserTimeline(self,
    ...
    if since_id:
      try:
        ...
        raise WeiboError("since_id must be an integer")

    if max_id:
      try:
        parameters['max_id'] = long(max_id)
      except:
        raise WeiboError("max_id must be an integer")
  ...
  def GetStatus(self, id):
    ...
    try:
      ...
      raise WeiboError("id must be an long integer")
  ...
  def DestroyStatus(self, id):
    ...
    try:
      ...
      raise WeiboError("id must be an integer")
  ...
  def PostUpdate(self, status, in_reply_to_status_id=None):
    ...
    if not self._oauth_consumer:
      raise WeiboError("The twitter.Api instance must be authenticated.")
    ...
    if len(u_status) > CHARACTER_LIMIT:
      raise WeiboError("Text must be less than or equal to %d characters. "
  ...
  def GetRetweets(self, statusid):
    ...
    '''
    if not self._oauth_consumer:
      raise WeiboError("The twitter.Api instsance must be authenticated.")
  ...
  def GetFriends(self, user=None, cursor=-1):
    ...
    '''
    if not user and not self._oauth_consumer:
      raise WeiboError("twitter.Api instance must be authenticated")
    if user:
  ...
  def UsersLookup(self, user_id=None, screen_name=None, users=None):
    ...
    if not user_id and not screen_name and not users:
      raise WeiboError("Specify at least on of user_id, screen_name, or users.")
  ...
  def VerifyCredentials(self):
    ...
    '''
    if not self._oauth_consumer:
      raise WeiboError("Api instance must first be given user credentials.")
  ...
  def _ParseAndCheckTwitter(self, json):
    ...
    try:
      ...
      self._CheckForWeiboError(data)
    except ValueError:
      if "<title>Twitter / Over capacity</title>" in json:
        raise WeiboError("Capacity Error")
      if "<title>Twitter / Error</title>" in json:
        raise WeiboError("Technical Error")
      raise WeiboError("json decoding")

    return data

  def _CheckForWeiboError(self, data):
    """Raises a WeiboError if twitter returns an error message.
    ...
    Raises:
      WeiboError wrapping the twitter error message if one exists.
    """
    # Twitter errors are relatively unlikely, so it is faster
    # to check first, rather than try and catch the exception
    if 'error' in data:
      raise WeiboError(data['error'])
